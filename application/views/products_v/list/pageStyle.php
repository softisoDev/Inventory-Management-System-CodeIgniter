<!--  DataTable Style  -->
<link rel="stylesheet" type="text/css" href="<?php echo base_url('app-assets'); ?>/vendors/css/tables/datatable/datatables.min.css">
<link rel="stylesheet" type="text/css" href="<?php echo base_url('app-assets'); ?>/vendors/css/tables/extensions/buttons.dataTables.min.css">
<link rel="stylesheet" type="text/css" href="<?php echo base_url('app-assets'); ?>/vendors/css/tables/datatable/buttons.bootstrap4.min.css">
<link rel="stylesheet" type="text/css" href="<?php echo base_url('app-assets'); ?>/vendors/css/tables/extensions/fixedColumns.dataTables.min.css">
<link rel="stylesheet" type="text/css" href="<?php echo base_url('app-assets'); ?>/vendors/css/tables/extensions/dataTables.colVis.css">
<!--<link rel="stylesheet" type="text/css" href="<?php /*echo base_url('app-assets'); */?>/vendors/css/tables/estensions/colReorder.dataTables.min.css">-->
<!--<link rel="stylesheet" type="text/css" href="<?php /*echo base_url('app-assets'); */?>/vendors/css/tables/datatable/select.dataTables.min.css">-->
<!--<link rel="stylesheet" type="text/css" href="<?php /*echo base_url('app-assets'); */?>/vendors/css/tables/extensions/fixedHeader.dataTables.min.css">-->
<link rel="stylesheet" type="text/css" href="<?php echo base_url('app-assets'); ?>/css/pages/products-list.css">