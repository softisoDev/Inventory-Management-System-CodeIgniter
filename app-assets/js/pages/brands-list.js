var drawTable = $('#brandsTable').DataTable( {
    "language": {
    ...
    "serverSide": true,
    "ajax": {
        url: app.host+"brands/getDataTable",
        type:"POST"
    },
    ...
    initComplete:function(){
        $('#createNew').html('<a href="'+app.host+'brands/add-brand" target="_blank" type="button" class="btn btn-info btn-sm"><i class="fa fa-plus"></i> Marka Əlavə Et</a>');
    },
    buttons: [
        {
            extend:'collection',
            ...
            buttons:[
                {
                    ...
                    action: function ( e, dt, node, config ) {
                        ...
                        window.location.href=app.host+"brands/update-brand/"+dataID;
                    }
                },
                {
                    ...
                    action: function ( e, dt, node, config ) {
                        var get_row = drawTable.rows({ selected: true }).nodes();
                        var dataID = $(get_row[0]).data('id');
                        removeData("brands/delete/",dataID);
                    }
                }
            ],
        },
