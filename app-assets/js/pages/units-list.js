var drawTable = $('#unitsTable').DataTable( {
    "language": {
    ...
    "serverSide": true,
    "ajax": {
        url: app.host+"units/getDataTable",
        type:"POST"
    },
    ...
    initComplete:function(){
        $('#createNew').html('<a href="'+app.host+'units/add-unit" target="_blank" type="button" class="btn btn-info btn-sm"><i class="fa fa-plus"></i> Ölçü vahidi Əlavə Et</a>');
    },
    buttons: [
        {
            extend:'collection',
            ...
            buttons:[
                {
                    ...
                    action: function ( e, dt, node, config ) {
                        ...
                        window.location.href=app.host+"units/update-unit/"+dataID;
                    }
                },
                {
                    ...
                    action: function ( e, dt, node, config ) {
                        var get_row = drawTable.rows({ selected: true }).nodes();
                        var dataID = $(get_row[0]).data('id');
                        removeData("units/delete/",dataID);
                    }
                }
            ],
        },
