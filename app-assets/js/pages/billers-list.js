var drawTable = $('#billersTable').DataTable( {
    "language": {
    ...
    columnDefs:[
        ...
        },
        {
            ...
            "data": function ( row, type, val, meta ) {
                ...
                var dataID = row[0];
                var columnData = '<input type="checkbox" data-url="billers/isActiveSetter/'+dataID+'" class="js-switch" '+val+' data-size="xs"/><label for="changeDataStatus" class="font-medium-2 text-bold-600 ml-1"></label>';
                return columnData;
            }
        }
    ],
    ...
    "serverSide": true,
    "ajax": {
        url: app.host+"billers/getDataTable",
        type:"POST"
    },
    ...
    initComplete:function(){
        $('#createNew').html('<a href="'+app.host+'billers/add-biller" target="_blank" type="button" class="btn btn-info btn-sm"><i class="fa fa-plus"></i> Fatura hazırlayan Əlavə Et</a>');
    },
    buttons: [
        {
            extend:'collection',
            ...
            buttons:[
                ...
                },
                {
                    ...
                    action: function ( e, dt, node, config ) {
                        var get_row = drawTable.rows({ selected: true }).nodes();
                        var dataID = $(get_row[0]).data('id');
                        window.location.href=app.host+"/billers/update-biller/"+dataID;
                    }
                },
                {
                    ...
                    action: function ( e, dt, node, config ) {
                        var get_row = drawTable.rows({ selected: true }).nodes();
                        var dataID = $(get_row[0]).data('id');
                        removeData("billers/delete/",dataID);
                    }
                }
            ],
        },
