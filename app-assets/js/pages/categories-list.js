var drawTable = $('#categoriesTable').DataTable( {
    "language": {
    ...
    "serverSide": true,
    "ajax": {
        url: app.host+"categories/getDataTable",
        type:"POST"
    },
    ...
    initComplete:function(){
        $('#createNew').html('<a href="'+app.host+'categories/add-category" target="_blank" type="button" class="btn btn-info btn-sm"><i class="fa fa-plus"></i> Kateqoriya Əlavə Et</a>');
    },
    buttons: [
        {
            extend:'collection',
            ...
            buttons:[
                {
                    ...
                    action: function ( e, dt, node, config ) {
                        ...
                        window.location.href=app.host+"categories/update-category/"+dataID;
                    }
                },
                {
                    ...
                    action: function ( e, dt, node, config ) {
                        var get_row = drawTable.rows({ selected: true }).nodes();
                        var dataID = $(get_row[0]).data('id');
                        removeData("categories/delete/",dataID);
                    }
                }
            ],
        },
