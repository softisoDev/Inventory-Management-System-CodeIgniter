var drawTable = $('#tasksTable').DataTable( {
    "language": {
    ...
    columnDefs:[
        ...
        },
        {
            ...
            "data": function ( row, type, val, meta ) {
                if(type==='display'){
                    var colVal = row[10]==1 ? "checked" : "unchecked";
                }
                let  dataID = row[0];
                let  columnData = '<input type="checkbox" data-url="tasks/isActiveSetter/'+dataID+'" class="js-switch" '+colVal+' data-size="xs"/><label for="changeDataStatus" class="font-medium-2 text-bold-600 ml-1"></label>';
                return columnData;
            }
        },
        {
            ...
            "data": function ( row, type, val, meta ) {
                ...
                if(type==='display'){
                    if(row[9]==1){
                        columnData = '<span class="badge badge-pill badge-success">Bitib</span>';
                    }
                    else{
                        columnData = '<span class="badge badge-pill badge-primary">Davam edir..</span>';
                    }
                }
                return columnData;
            }
    ...
    "serverSide": true,
    "ajax": {
        url: app.host+"tasks/getDataTable",
        type:"POST"
    },
    ...
    initComplete:function(){
        $('#createNew').html('<a href="'+app.host+'tasks/add-task" target="_blank" type="button" class="btn btn-info btn-sm"><i class="fa fa-plus"></i> Task Əlavə Et</a>');
    },
    buttons: [
        {
            extend:'collection',
            ...
            buttons:[
                {
                    ...
                    action: function ( e, dt, node, config ) {
                        ...
                        window.location.href = app.host+"tasks/more/"+dataID;
                    }
                },
                {
                    ...
                    action: function ( e, dt, node, config ) {
                        ...
                        window.location.href=app.host+"tasks/update-task/"+dataID;
                    }
                },
                {
                    ...
                    action: function ( e, dt, node, config ) {
                        var get_row = drawTable.rows({ selected: true }).nodes();
                        var dataID = $(get_row[0]).data('id');
                        removeData("tasks/delete/",dataID);
                    }
                },
                {
                    text:'<i class="la la-check"></i> Taskı bitir',
                    className:".dt-delete-item",
                    action: function ( e, dt, node, config ) {
                        var get_row = drawTable.rows({ selected: true }).nodes();
                        var dataID = $(get_row[0]).data('id');
                        window.location.href = app.host+"tasks/makeIsDone/"+dataID;
                    }
                }
            ],
        },
