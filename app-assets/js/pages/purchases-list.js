var drawTable = $('#purchasesTable').DataTable( {
    "language": {
    ...
    "serverSide": true,
    "ajax": {
        url: app.host+"purchases/getDataTable",
        type:"POST"
    },
    dom: "<'row'<'col-md-6'l><'#addPurchase.col-md-6 text-right'>>" +
        "<'row'<'#toolbar_buttons.col-md-8'B><'col-md-4'f>>"+
        "rtip",
    initComplete:function(){
        $('#addPurchase').html('<a href="'+app.host+'purchases/add-purchase" target="_blank" type="button" class="btn btn-info btn-sm"><i class="fa fa-plus"></i> Faktura Yarat</a>');
    },
    buttons: [
        /*
        {
            ...
            buttons:[
                ...
                },
                {
                    ...
                    action: function ( e, dt, node, config ) {
                        var get_row = drawTable.rows({ selected: true }).nodes();
                        var dataID = $(get_row[0]).data('id');
                        window.location.href=app.host+"purchases/update-purchase/"+dataID;
                    }
                },
                {
                    ...
                    action: function ( e, dt, node, config ) {
                        var get_row = drawTable.rows({ selected: true }).nodes();
                        var dataID = $(get_row[0]).data('id');
                        removeData("purchases/delete/",dataID);
                    }
                }
            ],
        },
