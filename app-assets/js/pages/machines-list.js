var drawTable = $('#machinesTable').DataTable( {
    "language": {
    ...
    "serverSide": true,
    "ajax": {
        url: app.host+"machines/getDataTable",
        type:"POST"
    },
    ...
    initComplete:function(){
        $('#createNew').html('<a href="'+app.host+'machines/add-machine" target="_blank" type="button" class="btn btn-info btn-sm"><i class="fa fa-plus"></i> Makina Əlavə Et</a>');
    },
    buttons: [
        {
            extend:'collection',
            ...
            buttons:[
                ...
                },
                {
                    ...
                    action: function ( e, dt, node, config ) {
                        var get_row = drawTable.rows({ selected: true }).nodes();
                        var dataID = $(get_row[0]).data('id');
                        window.location.href=app.host+"machines/update-machine/"+dataID;
                    }
                },
                {
                    ...
                    action: function ( e, dt, node, config ) {
                        var get_row = drawTable.rows({ selected: true }).nodes();
                        var dataID = $(get_row[0]).data('id');
                        removeData("machines/delete/",dataID);
                    }
                }
            ],
        },
