var drawTable = $('#departmentsTable').DataTable( {
    "language": {
    ...
    "serverSide": true,
    "ajax": {
        url: app.host+"departments/getDataTable",
        type:"POST"
    },
    ...
    initComplete:function(){
        $('#createNew').html('<a href="'+app.host+'departments/add-department" target="_blank" type="button" class="btn btn-info btn-sm"><i class="fa fa-plus"></i> Şöbə Əlavə Et</a>');
    },
    buttons: [
        {
            extend:'collection',
            ...
            buttons:[
                {
                    ...
                    action: function ( e, dt, node, config ) {
                        ...
                        window.location.href=app.host+"departments/update-department/"+dataID;
                    }
                },
                {
                    ...
                    action: function ( e, dt, node, config ) {
                        var get_row = drawTable.rows({ selected: true }).nodes();
                        var dataID = $(get_row[0]).data('id');
                        removeData("departments/delete/",dataID);
                    }
                }
            ],
        },
